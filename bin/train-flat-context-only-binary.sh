...
bin/train.py models/binary \
    --mode transient \
    --model-cfg fully_connected='[100]' n_residual_blocks=2 n_hidden_residual=100 n_epoch=500 patience=50 samples_per_epoch=$n_train pickle_path=data/contexts-100-min-100-max-per-word.pkl use_context_model=true context_model_type="flat" use_char_model=false batch_normalization=true class_weight_exponent=1 vectors_path=data/vectors.pkl.gz batch_size=32 n_val_samples=$n_valid optimizer=Adam n_context_embed_dims=$n_context_embed_dims 
